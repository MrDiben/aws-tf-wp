...
resource "aws_instance" "ec2" {
  ...
  provisioner "file" {
    source      = "files/install-wordpress-apache.sh"
    destination = "/tmp/install-wordpress-apache.sh"
  ...
  provisioner "remote-exec" {
    inline = [
      "chmod +x /tmp/install-wordpress-apache.sh",
      "/tmp/install-wordpress-apache.sh",
    ]
